...
# define network
class BinaryConnectMnistMLP(Chain):
    ...
    def __call__(self, x):
        # x:input
        h = self.bfc0(x)  # BinaryConnectAffine layer
        ...
        h = self.bfc2(h)  # BinaryConnectAffine_3 layer
        h = self.bn2(h)  # BatchNormalization_3 layer
        h = F.relu(h)  # ReLU_3 layer
        h = self.bfc3(h)  # BinaryConnectAffine_4 layer
        h = self.bn3(h)  # BatchNormalization_3 layer
...
def main(use_gpu=0):
    ...
    serializers.save_npz('./models/binary_connect_mnist_MLP', model)
