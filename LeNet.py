...
# define network
class LeNetwork(Chain):

    def __init__(self):
        super(LeNetwork, self).__init__()
        with self.init_scope():
            self.conv0 = L.Convolution2D(None, 16, ksize=7, pad=0, stride=1, dilate=1)
            self.conv1 = L.Convolution2D(None, 30, ksize=3, pad=0, stride=1, dilate=1)
            self.fc0 = L.Linear(None, 150)
    ...
    def __call__(self, x):
        ...
        h = self.conv1(h)  # Convolution layer
        h = F.max_pooling_2d(h, ksize=2, stride=2, pad=0)  # MaxPooling layer
        h = F.tanh(h)  # Tanh_2 layer
...
def main(use_gpu=0):
    ...
    serializers.save_npz('./models/LeNet', model)
